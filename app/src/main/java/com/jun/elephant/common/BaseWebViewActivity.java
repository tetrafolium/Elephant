...
public abstract class BaseWebViewActivity extends BaseActivity {
  ...
  protected static class WebAppClient extends WebViewClient {
    ...
    public WebAppClient(Context context, MultiStateView multiStateView,
                        WebView contentView) {
    ...
    public boolean shouldOverrideUrlLoading(WebView view, String url) {
      ...
      if (uri.getHost().contains(Constants.PHPHUB_HOST)) {
        ...
        if (segments.size() > 0) {
          if (segments.containsKey(Constants.PHPHUB_TOPIC_PATH) &&
              !TextUtils.isEmpty(segments.get(Constants.PHPHUB_TOPIC_PATH))) {
            url = String.format("%s%s?id=%s", Constants.DEEP_LINK_PREFIX,
                                Constants.PHPHUB_TOPIC_PATH,
                                segments.get(Constants.PHPHUB_TOPIC_PATH));
          } else if (segments.containsKey(Constants.PHPHUB_USER_PATH) &&
                     !TextUtils.isEmpty(
                         segments.get(Constants.PHPHUB_USER_PATH))) {
            url = String.format("%s%s?id=%s", Constants.DEEP_LINK_PREFIX,
                                Constants.PHPHUB_USER_PATH,
                                segments.get(Constants.PHPHUB_USER_PATH));
          }
        }
      }
      ...
      if (url.contains(Constants.DEEP_LINK_PREFIX)) {
        ...
        //                WebViewPageActivity.getCallingIntent(context, webUrl);
          ...
          .subscribe(
              new Action1<String>() {
                ...
                public void call(String javascript) {
                  contentView.loadUrl(
                      javascript.replace("{platform}", PLATFORM));
                }
              },
