...
public class TopicListAdapter
    extends RecyclerView.Adapter<TopicListAdapter.TopicViewHolder> {
  ...
  public TopicViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    return new TopicViewHolder(LayoutInflater.from(mContext).inflate(
  ...
  public void onBindViewHolder(TopicViewHolder holder, int position) {
    ...
    if (item.getLastReplyUser() != null) {
      temp.append(" • ").append("最后由").append(
          item.getLastReplyUser().getData().getName());
    }
    ...
    if (item.getUpdatedAt() != null) {
      ...
      try {
        String prettyTimeString =
            mPrettyTime.format(mDateFormat.parse(dateStr));
    ...
    holder.mBgaRl.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        mContext.startActivity(
            TopicDetailsActivity.newIntent(mContext, item.getId()));
      }
    });
    ...
    holder.mUserImg.setOnClickListener(new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        mContext.startActivity(UserInfoActivity.newIntent(
            mContext, Integer.parseInt(item.getUser().getData().getId())));
      }
    });
  }
