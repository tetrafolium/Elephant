...
/**
 * View that contains 4 different states: Content, Error, Empty, and
 * Loading.<br> Each state has their own separate layout which can be
 * shown/hidden by setting the {@link ViewState} accordingly Every
 * MultiStateView <b><i>MUST</i></b> contain a content view. The content view is
 * obtained from whatever is inside of the tags of the view via its XML
 * declaration
 */
public class MultiStateView extends FrameLayout {
  ...
  @IntDef({VIEW_STATE_CONTENT, VIEW_STATE_ERROR, VIEW_STATE_EMPTY,
           VIEW_STATE_LOADING})
  ...
  public MultiStateView(Context context) { this(context, null); }
  ...
  private void init(AttributeSet attrs) {
    ...
    int viewState =
        a.getInt(R.styleable.MultiStateView_msv_viewState, VIEW_STATE_CONTENT);
  ...
  /* All of the addView methods have been overridden so that it can obtain the
     content view via XML It is NOT recommended to add views into MultiStateView
     via the addView methods, but rather use
     any of the setViewForState methods to set views for their given ViewState
     accordingly */
  @Override
   ...
   * com.kennyc.view.MultiStateView.ViewState}
   *
   * @param state The {@link com.kennyc.view.MultiStateView.ViewState} with to
   *     return the view for
   * @return The {@link View} associated with the {@link
   *     com.kennyc.view.MultiStateView.ViewState}, null if no view is present
   */
  @Nullable
  ...
  private void setView() {
    ...
    default:
      if (mContentView == null) {
        // Should never happen, the view should throw an exception if no content
        // view is present upon creation
   ...
   * @param state         The {@link com.kennyc.view.MultiStateView.ViewState}to
   *     set
   * @param switchToState If the {@link
   *     com.kennyc.view.MultiStateView.ViewState} should be switched to
   */
   ...
   * @param state         The {@link com.kennyc.view.MultiStateView.ViewState}
   *     to set
   * @param switchToState If the {@link
   *     com.kennyc.view.MultiStateView.ViewState} should be switched to
   */
