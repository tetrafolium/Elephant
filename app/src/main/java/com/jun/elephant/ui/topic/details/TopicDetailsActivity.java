 ...
 */
public class TopicDetailsActivity
    extends BaseFrameWebViewActivity<TopicDetailsPresenter, TopicDetailsModel>
    implements Toolbar.OnMenuItemClickListener,
               VoteDialog.OnVoteDialogClickListener, TopicDetailsContract.View {

  @BindView(R.id.toolBar) Toolbar mToolBar;
  @BindView(R.id.webView) WebView mWebView;
  @BindView(R.id.point_tv) TextView mPointTv;
  @BindView(R.id.comment_tv) TextView mCommentTv;
  @BindView(R.id.favorite_tv) TextView mFavoriteTv;
  @BindView(R.id.follow_tv) TextView mFollowTv;
  @BindView(R.id.user_img_iv) MySimpleDraweeView mUserImgIv;
  @BindView(R.id.user_name_tv) TextView mUserNameTv;
  @BindView(R.id.user_desc_tv) TextView mUserDescTv;
  ...
  private boolean
      isLoadFinish; //记录网络加载是否完成，即是否成功取回帖子详情数据
  ...
  private void initViewDetail(TopicEntity mTopicEntity) {
    ...
    if (!TextUtils.isEmpty(
            mTopicEntity.getUser().getData().getIntroduction())) {
    ...
    mWebView.setWebViewClient(
        new WebAppClient(this, mMultiStateView, mWebView));
  ...
  @OnClick({R.id.point_tv, R.id.comment_tv, R.id.favorite_tv, R.id.follow_tv,
            R.id.user_img_iv})
  public void
  onClick(View view) {
    if (!isLoadFinish)
      return; //若还没有加载完成阻止继续下面操作
    ...
    case R.id.comment_tv:
      startActivity(TopicCommentListActivity.newIntent(
          this, mTopicEntity.getLinks().getRepliesWebView(),
          mTopicEntity.getId()));
      break;
    case R.id.user_img_iv:
      startActivity(UserInfoActivity.newIntent(
          this, Integer.parseInt(mTopicEntity.getUser().getData().getId())));
      break;
    }
  }
  ...
  public boolean onMenuItemClick(MenuItem item) {
    ...
    case R.id.action_share:
      if (isLoadFinish)
        ShareUtil.share(
            this, "分享 " + mTopicEntity.getUser().getData().getName() +
                      " 的文章" + mTopicEntity.getTitle() + " " +
  ...
  public void onVoteDownClick() {
    mPresenter.optStatusType(Constants.TopicOpt.TOPIC_VOTE_DOWN,
                             mTopicEntity.getId());
  }

  @Override
  public void onVoteUpClick() {
    mPresenter.optStatusType(Constants.TopicOpt.TOPIC_VOTE_UP,
                             mTopicEntity.getId());
  }
