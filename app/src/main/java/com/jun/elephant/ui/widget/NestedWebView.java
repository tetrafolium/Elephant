...
public class NestedWebView extends WebView implements NestedScrollingChild {
  ...
  public ScrollStateChangedListener.ScrollState position =
      ScrollStateChangedListener.ScrollState.TOP;
  ...
  public NestedWebView(Context paramContext) { this(paramContext, null); }

  public NestedWebView(Context context, @Nullable AttributeSet attrs) {
    this(context, attrs, 0);
  }

  public NestedWebView(Context paramContext, AttributeSet paramAttributeSet,
                       int paramInt) {
  ...
  private void getEmbeddedParent(View paramView) {
    ...
    if (localViewParent != null) {
      if ((localViewParent instanceof ScrollStateChangedListener)) {
        ...
        setScrollStateChangedListener(
            (ScrollStateChangedListener)localViewParent);
      } else {
  ...
  private void init() {
    ...
    //        addJavascriptInterface(new JSGetContentHeight(),
    //        "InjectedObject");
    Log.i(TAG, "max -- min Velocity = " + this.mMaximumVelocity + " -- " +
                   this.mMinimumVelocity + " touchSlop = " + this.mTouchSlop);
  }

  private void setJavaScriptEnable(boolean flag) {
    if (this.settings.getJavaScriptEnabled() != flag) {
      Log.i(TAG, "setJavaScriptEnable : " +
                     this.settings.getJavaScriptEnabled() + " / " + flag);
      this.settings.setJavaScriptEnabled(flag);
    }
  }

  private void
  setScrollStateChangedListener(ScrollStateChangedListener paramc) {
  ...
  public boolean dispatchNestedFling(float velocityX, float velocityY,
                                     boolean consumed) {
    return this.mChildHelper.dispatchNestedFling(velocityX, velocityY,
                                                 consumed);
  }
  ...
  public boolean dispatchNestedPreScroll(int dx, int dy, int[] consumed,
                                         int[] offsetInWindow) {
    return this.mChildHelper.dispatchNestedPreScroll(dx, dy, consumed,
                                                     offsetInWindow);
  }
  ...
  public boolean dispatchNestedScroll(int dxConsumed, int dyConsumed,
                                      ...
                                      int[] offsetInWindow) {
    return this.mChildHelper.dispatchNestedScroll(
        dxConsumed, dyConsumed, dxUnconsumed, dyUnconsumed, offsetInWindow);
  }

  public int getWebContentHeight() { return this.contentHeight; }
  ...
  public void invalidate() {
    ...
    if (this.contentHeight != this.preContentHeight) {
      loadUrl(
          "javascript:window.InjectedObject.getContentHeight(document.getElementsByTagName('div')[0].scrollHeight)");
      this.preContentHeight = this.contentHeight;
    }
  }

  public boolean isBeingDragged() { return this.mIsBeingDragged; }
  ...
  protected void onScrollChanged(int x, int y, int oldx, int oldy) {
    ...
    if (this.onScrollChangeListener != null) {
      this.onScrollChangeListener.onScrollChanged(x, y, oldx, oldy,
                                                  this.position);
    } else {
      //            Log.i(TAG,"yy=="+y+"  webviewHeight=="+this.webviewHeight+"
  ...
  public boolean onTouchEvent(MotionEvent ev) {
    ...
    case MotionEvent.ACTION_MOVE:
      ...
      //                }
      final int y =
          (int)(MotionEventCompat.getY(ev, activePointerIndex) + 0.5f);
      Log.i(TAG, "mLastMotionY=====" + mLastMotionY);
      Log.i(TAG, "YYYYYYY=====" + y);
      int deltaY = mLastMotionY - y;

      if (deltaY != 0) {
        this.direction = this.directionDetector.getDirection(
            deltaY, true, this.scrollStateChangedListener);
      }
      ...
      if (mIsBeingDragged) {
        ...
        Log.i(TAG, " child consumed = " + this.mScrollConsumed[1] +
                       " un_consumed = " + unconsumedY + " position = " +
        ...
        case 1: {
          if ((this.position !=
               ScrollStateChangedListener.ScrollState.BOTTOM) &&
              (this.contentHeight != this.webviewHeight)) {
            scrollBy(0, unconsumedY);
            break;
          }
          Log.i(TAG, "1111111consumedY===" + consumedY +
                         "  unconsumedY==" + unconsumedY);
          if (dispatchNestedScroll(0, this.consumedY, 0, unconsumedY,
                                   this.mScrollOffset)) {
        ...
        case 2:
          if ((this.position == ScrollStateChangedListener.ScrollState.TOP) ||
              (this.contentHeight == this.webviewHeight)) {
            Log.i(TAG, "2222222consumedY===" + consumedY +
                           "  unconsumedY==" + unconsumedY);
            if (dispatchNestedScroll(0, this.consumedY, 0, unconsumedY,
                                     this.mScrollOffset)) {
              vtev.offsetLocation(0.0F, this.mScrollOffset[1]);
    ...
    case MotionEventCompat.ACTION_POINTER_UP:
      ...
      mLastMotionY =
          (int)(MotionEventCompat.getY(ev, MotionEventCompat.findPointerIndex(
                                               ev, mActivePointerId)) +
                0.5F);
      break;
    }
  ...
  private void onSecondaryPointerUp(MotionEvent ev) {
    final int pointerIndex =
        (ev.getAction() & MotionEventCompat.ACTION_POINTER_INDEX_MASK) >>
  ...
  protected boolean overScrollBy(int deltaX, int deltaY, int scrollX,
    ...
    return super.overScrollBy(deltaX, deltaY, scrollX, scrollY, scrollRangeX,
                              scrollRangeY, maxOverScrollX, maxOverScrollY,
                              isTouchEvent);
  }
  ...
  public void scrollToTop() { scrollTo(getScrollX(), 0); }

  public void setContentHeight(int contentHeight) {
    this.contentHeight = contentHeight;
    Log.i(TAG, "contentHeight = " + contentHeight +
                   " -  webviewHeight = " + this.webviewHeight + " = " +
                   (contentHeight - this.webviewHeight));
  }
  ...
  public void setWebViewClient(WebViewClient paramWebViewClient) {
    if (!(paramWebViewClient instanceof WebViewClient))
      throw new IllegalArgumentException(
          "WebViewClient should be instance of EmbeddedWebView$WebViewClient");
  ...
  public static interface OnScrollChangeListener {
    ...
    onScrollChanged(int paramInt1, int paramInt2, int paramInt3, int paramInt4,
                    ScrollStateChangedListener.ScrollState parama);
  }
}
