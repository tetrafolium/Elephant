 ...
 */
public class TopicListByForumFragment
    extends BaseFrameFragment<TopicPresenter, TopicModel>
    implements TopicContract.View {

  @BindView(R.id.recyclerView) RecyclerView mRecyclerView;
  @BindView(R.id.swipe_layout) MySwipeRefreshLayout mSwipeLayout;
  @BindView(R.id.multiStateView) MultiStateView mMultiStateView;
  @BindView(R.id.fab) FloatingActionButton mFabBtn;
  ...
  public void getData() {
    ...
    if (state == MultiStateView.VIEW_STATE_EMPTY ||
        state == MultiStateView.VIEW_STATE_ERROR) {
      mPageIndex = 1;
      mMultiStateView.setViewState(MultiStateView.VIEW_STATE_LOADING);
      mPresenter.getTopicListByForum(TYPE, mPageIndex);
      return;
  ...
  public void refreshUI() {
    ...
    for (int childIndex = 0; childIndex < childCount; childIndex++) {
      ...
      BGABadgeRelativeLayout bgarl =
          (BGABadgeRelativeLayout)childView.findViewById(R.id.bgaRl);
      bgarl.getBadgeViewHelper().setBadgeBgColorInt(
          ContextCompat.getColor(getContext(), badgeColor.resourceId));
    }
    ...
    //那么，如果是ListView，要怎么做呢？这里的思路是通过反射拿到 AbsListView
    //类中的 RecycleBin 对象，然后同样再用反射去调用 clear 方法
    Class<RecyclerView> recyclerViewClass = RecyclerView.class;
    try {
      Field declaredField = recyclerViewClass.getDeclaredField("mRecycler");
      declaredField.setAccessible(true);
      Method declaredMethod =
          Class.forName(RecyclerView.Recycler.class.getName())
