...
public class TopicModel implements TopicContract.Model {
  ...
  public Observable<TokenEntity> getTokenByForum() {
    return Networks.getInstance()
        .getTokenApi()
        .getToken(Constants.Token.AUTH_TYPE_GUEST, BuildConfig.CLIENT_ID,
                  BuildConfig.CLIENT_SECRET)
        .compose(RxSchedulers.<TokenEntity>io_main());
  }

  @Override
  public Observable<TopicListEntity> getTopicByForum(String type,
                                                     int pageIndex) {
    return Networks.getInstance()
        .getTopicApi()
   ...
   */
  @Override
  public Observable<TopicListEntity> getUserTopicPraise(int userId,
                                                        int pageIndex) {
    return Networks.getInstance()
        .getUserApi()
   ...
   */
  @Override
  public Observable<TopicListEntity> getUserTopicShare(int userId,
                                                       int pageIndex) {
    return Networks.getInstance()
        .getUserApi()
   ...
   */
  @Override
  public Observable<TopicListEntity> getUserFollowUser(int userId,
                                                       int pageIndex) {
    return Networks.getInstance().getUserApi().getAttentions(userId).compose(
        RxSchedulers.<TopicListEntity>io_main());
  }
