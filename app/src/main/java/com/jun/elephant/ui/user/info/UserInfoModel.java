...
public class UserInfoModel implements UserInfoContract.Model {
  ...
  public Observable<UserInfoEntity> getUserInfoById(int userId) {
    return Networks.getInstance().getUserApi().getUserInfoById(userId).compose(
        RxSchedulers.<UserInfoEntity>io_main());
  }
   ...
   */
  @Override
  public Observable<UserInfoEntity> saveUserInfoById(int userId,
                                                     UserEntity userEntity) {
    return Networks.getInstance()
        .getUserApi()
        .saveUserInfo(userId, userEntity)
