...
public class PtrFrameLayout extends ViewGroup {
  ...
  public enum Mode { NONE, REFRESH, LOAD_MORE, BOTH }
  ...
  public PtrFrameLayout(Context context) { this(context, null); }
  ...
  public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    TypedArray arr =
        context.obtainStyledAttributes(attrs, R.styleable.PtrFrameLayout, 0, 0);
    if (arr != null) {
      mHeaderId =
          arr.getResourceId(R.styleable.PtrFrameLayout_ptr_header, mHeaderId);
      mContainerId = arr.getResourceId(R.styleable.PtrFrameLayout_ptr_content,
                                       mContainerId);
      mFooterId =
          arr.getResourceId(R.styleable.PtrFrameLayout_ptr_footer, mFooterId);

      mPtrIndicator.setResistance(
          arr.getFloat(R.styleable.PtrFrameLayout_ptr_resistance,
      ...
      mDurationToCloseHeader =
          arr.getInt(R.styleable.PtrFrameLayout_ptr_duration_to_close_header,
                     mDurationToCloseHeader);
      ...
      mPullToRefresh = arr.getBoolean(
          R.styleable.PtrFrameLayout_ptr_pull_to_fresh, mPullToRefresh);

      mMode =
          getModeFromIndex(arr.getInt(R.styleable.PtrFrameLayout_ptr_mode, 4));
  ...
  protected void onFinishInflate() {
    ...
    if (childCount > 3) {
      throw new IllegalStateException(
          "PtrFrameLayout only can host 3 elements");
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    if (mFooterView != null) {
      measureChildWithMargins(mFooterView, widthMeasureSpec, 0,
                              heightMeasureSpec, 0);
  ...
  private void layoutChildren() {
    // because the header and footer can not show at the same time, so when
    // header has a offset, the footer's offset should be 0, vice versa..
    int offsetHeaderY;
    int offsetFooterY;
    ...
    if (DEBUG && DEBUG_LAYOUT) {
      PtrCLog.d(LOG_TAG, "onLayout offset: %s %s %s %s", offsetHeaderY,
                offsetFooterY, isPinContent(), mPtrIndicator.isHeader());
    }
    ...
    if (mHeaderView != null) {
      ...
      if (DEBUG && DEBUG_LAYOUT) {
        PtrCLog.d(LOG_TAG, "onLayout header: %s %s %s %s %s", left, top, right,
                  bottom, mHeaderView.getMeasuredHeight());
      }
    }
    if (mContent != null) {
      ...
      if (DEBUG && DEBUG_LAYOUT) {
        PtrCLog.d(LOG_TAG, "onLayout content: %s %s %s %s %s", left, top, right,
                  bottom, mContent.getMeasuredHeight());
      }
    ...
    if (mFooterView != null) {
      ...
      final int top = paddingTop + lp.topMargin + contentBottom -
                      (isPinContent() ? offsetFooterY : 0);
      final int right = left + mFooterView.getMeasuredWidth();
      final int bottom = top + mFooterView.getMeasuredHeight();
      mFooterView.layout(left, top, right, bottom);
      if (DEBUG && DEBUG_LAYOUT) {
        PtrCLog.d(LOG_TAG, "onLayout footer: %s %s %s %s %s", left, top, right,
                  bottom, mFooterView.getMeasuredHeight());
      }
    }
  }
    ...
    case MotionEvent.ACTION_MOVE:
      ...
      if ((mDisableWhenHorizontalMove && !mPreventForHorizontal &&
          (Math.abs(offsetX) > mPagingTouchSlop &&
           Math.abs(offsetX) > Math.abs(offsetY))) && (mPtrIndicator.isInStartPosition())) {
      ...
      boolean canMoveUp =
          mPtrIndicator.isHeader() &&
          mPtrIndicator.hasLeftStartPosition(); // if the header is showing

      boolean canMoveDown =
          mFooterView != null && !mPtrIndicator.isHeader() &&
          mPtrIndicator.hasLeftStartPosition(); // if the footer is showing

      boolean canHeaderMoveDown =
          mPtrHandler != null &&
          mPtrHandler.checkCanDoRefresh(this, mContent, mHeaderView) &&
          (mMode.ordinal() & 1) > 0;
      boolean canFooterMoveUp =
          mPtrHandler != null &&
          mFooterView !=
              null // The footer view could be null, so need double check
          && mPtrHandler instanceof PtrHandler2 &&
          ((PtrHandler2)mPtrHandler)
              .checkCanDoLoadMore(this, mContent, mFooterView) &&
          (mMode.ordinal() & 2) > 0;

      if (DEBUG) {
        PtrCLog.v(
            LOG_TAG,
            "ACTION_MOVE: offsetY:%s, currentPos: %s, moveUp: %s, canMoveUp: %s, moveDown: %s: canMoveDown: %s canHeaderMoveDown: %s canFooterMoveUp: %s",
            offsetY, mPtrIndicator.getCurrentPosY(), moveUp, canMoveUp,
            moveDown, canMoveDown, canHeaderMoveDown, canFooterMoveUp);
      }
      ...
      // When status is completing, that is, the footer is hiding, disable pull
      // up
  ...
  private void updatePos(int change) {
    ...
    if ((mPtrIndicator.hasJustLeftStartPosition() &&
   ...
   */
  private void tryScrollBackToTopWhileLoading() { tryScrollBackToTop(); }

  /**
   * just make easier to understand
   */
  private void tryScrollBackToTopAfterComplete() { tryScrollBackToTop(); }
  ...
  final public void refreshComplete() {
    ...
    int delay = (int)(mLoadingMinTime -
                      (System.currentTimeMillis() - mLoadingStartTime));
    if (delay <= 0) {
      if (DEBUG) {
  ...
  private void notifyUIRefreshComplete(boolean ignoreHook) {
    /**
     * After hook operation is done, {@link #notifyUIRefreshComplete} will be
     * call in resume action to ignore hook.
     */
  ...
  public void autoRefresh() { autoRefresh(true, mDurationToCloseHeader); }
  ...
  public boolean isAutoRefresh() { return (mFlag & MASK_AUTO_REFRESH) > 0; }
  ...
  public boolean isPinContent() { return (mFlag & FLAG_PIN_CONTENT) > 0; }
   ...
   * @param time
   */
  public void setLoadingMinTime(int time) { mLoadingMinTime = time; }
  ...
  public void setPtrHandler(PtrHandler ptrHandler) { mPtrHandler = ptrHandler; }
  ...
  public void removePtrUIHandler(PtrUIHandler ptrUIHandler) {
    mPtrUIHandlerHolder =
        PtrUIHandlerHolder.removeHandler(mPtrUIHandlerHolder, ptrUIHandler);
  }
   ...
   * @param duration
   */
  public void setDurationToClose(int duration) { mDurationToClose = duration; }
  ...
  private void sendCancelEvent() {
    ...
    // mLastMoveEvent is null. fix #104, #80, #92
  ...
  private void sendDownEvent() {
    ...
    MotionEvent e = MotionEvent.obtain(last.getDownTime(), last.getEventTime(),
                                       MotionEvent.ACTION_DOWN, last.getX(),
  ...
  class ScrollChecker implements Runnable {
    ...
    public ScrollChecker() { mScroller = new Scroller(getContext()); }

    public void run() {
      boolean finish =
          !mScroller.computeScrollOffset() || mScroller.isFinished();
      ...
      if ((DEBUG) && (deltaY != 0)) {
        PtrCLog.v(
            ...
            finish, mStart, mTo, mPtrIndicator.getCurrentPosY(), curY,
            mLastFlingY, deltaY);
      }
      if (!finish) {
        mLastFlingY = curY;
    ...
    public boolean isRunning() { return mScroller.isFinished(); }
    ...
    public void tryToScrollTo(int to, int duration) {
      ...
      if (DEBUG) {
        PtrCLog.d(LOG_TAG, "tryToScrollTo: start: %s, distance:%s, to:%s",
                  mStart, distance, to);
      }
      removeCallbacks(this);
