...
public class MarkdownTextChangeWatcher implements TextWatcher {
  ...
  public void afterTextChanged(Editable s) {
    ...
    List<MarkdownSyntaxModel> models =
        MarkdownSyntaxGenerator.syntaxModelsForString(s.toString());
    if (models.size() == 0) {
      return;
    }
    mLastStyle.clear();
    for (MarkdownSyntaxModel model : models) {
      ...
      //            MarkdownSyntaxGenerator.styleFromSyntaxType(type);
      int low = range.getLower();
      int upper = range.getUpper();
      //            mLastStyle.add(style);
      //            s.setSpan(style, low, upper,
